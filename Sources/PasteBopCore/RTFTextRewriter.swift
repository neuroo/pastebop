...
enum RTFTextRewriter {
    ...
    /// The rewritten document, or `nil` if no text needed rewriting.
    static func rewrite(_ rtf: Data, rules: RewriteRules = .builtIn) -> Data? {
        guard !rtf.isEmpty else { return nil }
        return rtf.withUnsafeBytes { raw -> Data? in
            var rewriter = RTFRewriter(source: raw.bindMemory(to: UInt8.self), table: rules.table)
            return rewriter.run().map { Data($0) }
        }
    }
}
...
private struct RTFRewriter {
    ...
    init(source: UnsafeBufferPointer<UInt8>, table: ScalarTable) {
        self.source = source
        self.table = table
    }
